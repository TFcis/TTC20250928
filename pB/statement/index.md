...
還有一點點不同的地方，因為 Madfarm 有點智障，所以差分的方向是 **相反的**

但 Madfarm 實在太快了 所以你也要盡可能的快且精準的求出答案，又因為他的加加加加速度可能太大太大了，所以要對某個神奇數字 $P$ 取模，又因為 Madfarm 不可能倒著開車，所以他的加加加加速度一定是正的
...
## 輸出
輸出 $N-1$ 次差分後的結果並模P (差分到最後一個數字)

模 $P$ 之後若是負數，請加上 $P$ (即輸出模 $P$ 下的正值)

## 輸入限制
 - $1 \le T \le 1 * 10^5$
 - $1 \le N \le 1 * 10^6, \Sigma\ {N} \le 2 * 10^6$
 - $2 \le P \le 2 * 10^9, P$ 是質數
